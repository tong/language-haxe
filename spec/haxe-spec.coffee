...
describe "Haxe Grammar", ->
    ...
    beforeEach ->
        ...
        runs ->
            grammar = atom.grammars.grammarForScopeName("source.hx")

    it "parses the grammar", ->
        expect(grammar).toBeDefined()
        expect(grammar).toBeTruthy()
        expect(grammar.scopeName).toBe "source.hx"
    ...
    describe "keywords", ->
        ...
        for keyword in keywords
            it "tokenizes the #{keyword} keyword", ->
                ...
                # TODO
                expect(tokens[0]).toEqual value: keyword, scopes: ['source.hx','keyword.control.haxe.flow-control.2']

    describe "numbers", ->
        it "tokenizes hexadecimals", ->
            {tokens} = grammar.tokenizeLine('0x1D306')
            expect(tokens[0]).toEqual value: '0x1D306', scopes: ['source.hx', 'constant.numeric.haxe']
        it "tokenizes decimals", ->
            {tokens} = grammar.tokenizeLine('1234')
            expect(tokens[0]).toEqual value: '1234', scopes: ['source.hx', 'constant.numeric.haxe']
            ...
            expect(tokens[0]).toEqual value: '9', scopes: ['source.hx', 'constant.numeric.haxe']
            #expect(tokens[1]).toEqual value: '.', scopes: ['source.hx', 'constant.numeric.decimal.hx', 'meta.delimiter.decimal.period.hx']

    describe "switch statements", ->
        it "tokenizes the switch keyword", ->
            {tokens} = grammar.tokenizeLine('switch(){}')
            expect(tokens[0]).toEqual value: 'switch', scopes: ['source.hx', 'keyword.control.haxe.flow-control.2'] # TODO
        it "tokenizes switch expression", ->
            {tokens} = grammar.tokenizeLine('switch(foo + bar){}')
            expect(tokens[0]).toEqual value: 'switch', scopes: ['source.hx', 'keyword.control.haxe.flow-control.2'] # TODO
            expect(tokens[1]).toEqual value: '(', scopes: ['source.hx', 'meta.scope.field-completions.haxe']
            #expect(tokens[2]).toEqual value: 'foo ', scopes: ['source.hx', 'meta.scope.field-completions.haxe']
            #expect(tokens[3]).toEqual value: '+', scopes: ['source.hx', 'meta.scope.field-completions.haxe']
            #expect(tokens[4]).toEqual value: ' bar', scopes: ['source.hx', 'meta.switch-statement.js']
            #expect(tokens[5]).toEqual value: ')', scopes: ['source.hx', 'meta.switch-statement.js', 'punctuation.definition.switch-expression.end.bracket.round.js']

    describe "function calls", ->
        #it "tokenizes function calls", ->
            #{tokens} = grammar.tokenizeLine('functionCall()')
            #expect(tokens[0]).toEqual value: 'functionCall', scopes: ['source.hx']
            #expect(tokens[1]).toEqual value: '(', scopes: ['source.hx']
    ...
    describe "strings and functions", ->
        it "doesn't confuse them", ->
            ...
            for scope, delim of delimsByScope
                ...
                expect(tokens[0]).toEqual value: 'a', scopes: ['source.hx'] #,'meta.scope.field-completions.haxe']
                #expect(tokens[1]).toEqual value: '.', scopes: ['source.hx']
